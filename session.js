...
const SessionManager = (() => {
  ...
  // Method: z-score speed & accuracy within block, combine with block weights,
  // then **percentile-rank the composite** → uniform labels spread across [0.05, 0.95].
  // This avoids the old mistake of compressing most windows to 0.4–0.6.
  function finalizeBlockLabels() {
    ...
    const speeds = engaged.map(s => s.speedRaw);
    const accs   = engaged.map(s => s.accuracyRaw);
    ...
    // Block-specific combine weights for the composite z-score
    const wS = calibState.phase === 'typing' ? 0.5 : 0.3;
    const wA = calibState.phase === 'typing' ? 0.5 : 0.7;

    // Composite score per engaged sample
    const composite = engaged.map(s => {
      const zS = (s.speedRaw    - meanS) / sdS;
      const zA = (s.accuracyRaw - meanA) / sdA;
      return { s, c: zS * wS + zA * wA };
    });
    // Sort ascending and assign percentile-rank label in [0.05, 0.95]
    composite.sort((a, b) => a.c - b.c);
    const denom = Math.max(1, composite.length - 1);
    composite.forEach((item, i) => {
  ...
  function renderCalibStatus() {
    ...
    if (p.model && p.model.weights) {
      ...
      const blend = r2Held != null ? Math.max(0, Math.min(1, r2Held)) : (typeof cal.r2 === 'number' ? Math.max(0, Math.min(1, cal.r2)) : 0);
