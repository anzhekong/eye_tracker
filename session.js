...
const SessionManager = (() => {
  ...
  // Train ridge-regularized logistic regression with MSE loss (soft labels in [0,1]).
  // Batch gradient descent, vanilla JS, no deps. ~500 epochs on ≤200 samples = fast.
  function trainPersonalModel(samples, opts = {}) {
    ...
    const D = FEATURE_NAMES.length;
    const { mu, sigma } = standardizeFeatures(samples);

    // Cache standardized vectors so we don't re-standardize every epoch
    const X = samples.map(s => {
      const xs = new Array(D);
      for (let i = 0; i < D; i++) xs[i] = (s.features[i] - mu[i]) / (sigma[i] || 1);
      return xs;
    });
    const Y = samples.map(s => Math.max(0, Math.min(1, s.perf)));
    ...
    for (let e = 0; e < epochs; e++) {
      ...
      for (let n = 0; n < N; n++) {
        let z = b;
        for (let i = 0; i < D; i++) z += X[n][i] * w[i];
        const p    = sigmoid(z);
        const err  = p - Y[n];
        const dSig = p * (1 - p);
        const grad = err * dSig;
        for (let i = 0; i < D; i++) gW[i] += grad * X[n][i];
        gB += grad;
      }
    ...
    // Training quality: R² against the soft labels
    ...
    return {
      ...
      model: {
        ...
        calibration:  { nWindows: N, meanPerf: meanY, r2 },
      },
    };
  }
  ...
  function refinePersonalModel(model, newSamples, opts = {}) {
    ...
    for (let e = 0; e < epochs; e++) {
      for (const s of newSamples) {
        ...
        const p = sigmoid(z);
        const err = p - Math.max(0, Math.min(1, s.perf));
        const dSig = p * (1 - p);
        const grad = err * dSig;
    ...
    return Object.assign({}, model, { weights: w, bias: b, trainedAt: new Date().toISOString() });
  }
  ...
  function calcFocusScorePersonalModel(model, eyeMoves, blinkRate, headScore, extras) {
    ...
    const p = predictModel(model, features); // 0..1
  ...
  function feedbackOff(sessionId) {
    ...
    if (p.model && p.model.weights) {
      p.model = refinePersonalModel(p.model, newSamples);
    }
    p.calibrationSamples = (p.calibrationSamples || []).concat(newSamples);
    saveProfile(p);
    renderCalibStatus();
  ...
  const CALIB_BLOCKS       = ['typing', 'arith'];
    ...
    if (phase === 'intro') {
      body.innerHTML = `
        <div class="calib-card">
          ...
          <ul>
            ...
            <li><b>Block 2 · Arithmetic (5 min)</b> — solve mental-math problems.</li>
          </ul>
          <p>Your typing speed + accuracy and problem-solving speed + accuracy serve as <span class="calib-emph">objective labels</span>. We correlate every 10-second window with your webcam signals (gaze, blink, head) and train a personal logistic-regression model in your browser.</p>
      ...
      body.innerHTML = `
        <div class="calib-card" style="text-align:center">
          ...
          <p>Your personal attention model is live. Every future session will be scored against your own fingerprint.</p>
          <div class="calib-done-stats">
            ...
            <div class="calib-done-stat">
              <span class="calib-done-stat-val">${cal ? cal.r2.toFixed(2) : '—'}</span>
              <span class="calib-done-stat-label">R²</span>
            </div>
            <div class="calib-done-stat">
          ...
          <p style="color:var(--muted);font-size:0.8rem">R² measures how well your webcam behavior explains your task performance. Even 0.2–0.4 is meaningful with this little data.</p>
          <div style="margin-top:16px">
  ...
  function calibNext() {
    ...
    sampleGazeY     = [];
    if (block === 'typing') {
      calibState.typing.paragraphIdx       = 0;
      calibState.typing.target             = CALIB_PARAGRAPHS[0];
      calibState.typing.typed              = '';
      calibState.typing.committedTotal     = 0;
      calibState.typing.errorTotal         = 0;
      calibState.typing.lastWindowCommitted = 0;
      calibState.typing.lastWindowError     = 0;
    }
    if (block === 'arith') {
  ...
  // Turn raw per-window speed+accuracy into a soft [0,1] label per window,
  // z-scored within the block so different task scales don't conflate.
  function finalizeBlockLabels() {
    const samples = calibState.blockSamples.filter(s => s.engaged === 1);
    if (samples.length === 0) {
      // Everyone's been idle — give all windows a low label
      calibState.blockSamples.forEach(s => { s.perf = 0.1; });
      return;
    }
    const speeds    = samples.map(s => s.speedRaw);
    const accs      = samples.map(s => s.accuracyRaw);
    const meanS = speeds.reduce((a, b) => a + b, 0) / speeds.length;
    const meanA = accs  .reduce((a, b) => a + b, 0) / accs.length;
    const sdS = Math.sqrt(speeds.reduce((a, b) => a + (b - meanS) ** 2, 0) / speeds.length) || 1;
    const sdA = Math.sqrt(accs  .reduce((a, b) => a + (b - meanA) ** 2, 0) / accs.length)   || 1;

    // Block-specific combine weights
    const wS = calibState.phase === 'typing' ? 0.5 : 0.3;
    const wA = calibState.phase === 'typing' ? 0.5 : 0.7;

    const toUnit = (z) => {
      const clipped = Math.max(-2, Math.min(2, z));
      return (clipped + 2) / 4; // → [0,1]
    };
    ...
    for (const s of calibState.blockSamples) {
      ...
      const zS = (s.speedRaw    - meanS) / sdS;
      const zA = (s.accuracyRaw - meanA) / sdA;
      s.perf = toUnit(zS) * wS + toUnit(zA) * wA;
    }
  }
  ...
  function updateCalibTypingDisplay() {
    ...
    if (tgt) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        // Correct spaces: use a real space so the browser can break the line
        // here. white-space: pre-wrap in CSS keeps them from collapsing.
        // Wrong spaces: keep &nbsp; so the red error square is visually solid.
        const safe = isSpace
          ? (ok ? ' ' : '&nbsp;')
  ...
  function renderCalibStatus() {
    ...
    let html = '';
    if (p.model && p.model.weights) {
      const r2 = p.model.calibration ? p.model.calibration.r2.toFixed(2) : '—';
      const when = p.model.trainedAt ? new Date(p.model.trainedAt).toLocaleDateString() : '';
      html = `
        <div class="calib-status">
          <b>Personal model active</b><br>
          R² ${r2} · ${p.model.calibration ? p.model.calibration.nWindows : 0} windows · ${when}
